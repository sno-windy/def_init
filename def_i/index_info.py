...
class GetIndexInfo:
    ...
    def get_related_questions(self):
        related_questions = Question.objects.filter(lesson__course__category=self.studying_category).order_by('-created_at')[:5]
        return related_questions

    # 進行中のコースに関連したノートを取得
    def get_related_articles(self):
        related_articles = Article.objects.filter(lesson__course__category=self.studying_category).order_by('-created_at')[:5]  # 並べる順番
